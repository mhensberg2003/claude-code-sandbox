...
The Sandbox ships its **own** MCP config (never the Host's `~/.claude.json`), containing only an explicitly-permitted allowlist of low-risk, read-only servers. **v1 allowlist = context7** (doc lookup: `resolve-library-id`, `query-docs`). Claude Code wraps MCP server launches with `$CLAUDE_CODE_SHELL_PREFIX` just like Bash commands, so context7 actually runs as uid `runner` (proxy-only), **not** `claude` — it reaches `context7.com` through the registry allowlist (whitelisted there), so it stays deprivileged and adds **no new exfil channel** beyond the accepted 5c WebFetch risk. The only widening is that `runner` commands can now also reach `context7.com`. All credential-bearing MCP servers (Gmail, Linear, …) are absent because `mcpOAuth` is never injected.

**Toolchain: one batteries-included image.**
The image is the only toolchain (Host not mounted), so `:stable` bakes in node (npm + pnpm), python, go, rust, git, build-essential, ripgrep, and common CLIs. Chosen over slim per-stack variants so it "just works" across any project for the fleet. Future opt-in extension: a project-level `Dockerfile.sandbox` that `FROM`s the base (not v1).
